...
import { Check, Clock, ExternalLink, GripVertical, Trash2 } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center flex-shrink-0 pt-1">
        {draggable && (
          <GripVertical size={12} className="text-gray-300 mb-1 opacity-0 group-hover:opacity-100 transition-opacity"/>
        )}
      ...
      <div className={`flex-1 bg-white rounded-xl border p-3 mb-3 transition-all hover:shadow-sm ${
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <span className="px-2 py-0.5 rounded-md text-xs font-medium text-white flex-shrink-0" style={{ background: tc.color }}>
              {tc.emoji} {tc.label}
            </span>
        ...
        <div className="flex items-center gap-2 mt-2">
          {editingTime ? (
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => setEditingTime(false)} className="text-gray-400 text-xs">×</button>
            </div>
          ) : (
          ...
          {spot.rating && <span className="text-xs text-yellow-600 ml-auto">★ {spot.rating}</span>}
        </div>
